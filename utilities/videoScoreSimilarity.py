...
if __name__ == "__main__":
    ...
    for positionIdx, position in enumerate(tqdm(wayPointsID)):
        ...
        for time,time_step in enumerate(time_steps):
            ...
            file_score = os.path.join(information_dir, f"information_time_{positionIdx+time}.png")
            frame_score = cv2.imread(file_score)
            frame_score_resized = cv2.resize(frame_score,(1000,1000))
            ...
            frame = np.concatenate((frame_top,frame_bot), axis=0)
            out.write(frame)
