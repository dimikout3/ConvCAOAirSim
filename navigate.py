...
if __name__ == "__main__":
    ...
    d = {"UAV1":pickle.load(open("state_Drone1.pickle","rb")),
         "UAV2":pickle.load(open("state_Drone2.pickle","rb")),
         "UAV3":pickle.load(open("state_Drone3.pickle","rb")),
         "UAV4":pickle.load(open("state_Drone4.pickle","rb"))}

    alt = -15.

    for positionIdx in range(0,len(d["UAV1"])):

        ptime = time.time()

        tasks = []
        for ctrl in controllers:
            t = ctrl.moveToPositionYawModeAsync( d[ctrl.getName()][positionIdx][0].kinematics_estimated.position.x_val,
                                                 d[ctrl.getName()][positionIdx][0].kinematics_estimated.position.y_val,
                                                 alt, speed=3,
                                                 yawmode = np.degrees(airsim.to_eularian_angles(d[ctrl.getName()][positionIdx][0].kinematics_estimated.orientation)[2]) )
            tasks.append(t)
